package com.example.media.gsy.manager;
...
import com.example.media.gsy.factory.CacheFactory;
import com.example.media.gsy.factory.PlayerFactory;
import com.example.media.gsy.model.GSYModel;
import com.example.media.gsy.model.VideoOptionModel;
...
public abstract class GSYVideoBaseManager implements IMediaPlayer.OnPreparedListener, IMediaPlayer.OnCompletionListener,
    ...
    public void prepare(final String url, final Map<String, String> mapHeadData, boolean loop, float speed, boolean cache, File cachePath, String overrideExtension) {
        ...
        msg.obj = new GSYModel(url, mapHeadData, loop, speed, cache, cachePath, overrideExtension);
        sendMessage(msg);
        if (needTimeOutOther) {
            startTimeOutBuffer();
        }
    ...
    private Runnable mTimeOutRunnable = new Runnable() {
        ...
        public void run() {
            if (listener != null) {
                Log.e(TAG, "time out for error listener");
