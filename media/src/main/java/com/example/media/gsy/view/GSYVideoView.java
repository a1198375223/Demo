...
import com.example.media.gsy.common.GSYMediaPlayerListener;
import com.example.media.gsy.common.GSYVideoViewBridge;
import com.example.media.gsy.common.NetInfoModule;
import com.example.media.gsy.common.VideoAllCallBack;
import com.example.media.gsy.render.GSYTextureRenderView;
...
public abstract class GSYVideoView extends GSYTextureRenderView implements GSYMediaPlayerListener {
    private static final String TAG = "GSYVideoView";
    ...
    protected void startPrepare() {
        ...
        getGSYVideoManager().prepare(mUrl, (mMapHeadData == null) ? new HashMap<>() : mMapHeadData, mLooping, mSpeed, mCache, mCachePath, mOverrideExtension);
     ...
     * 监听是否有外部其他多媒体开始播放
     */
    protected AudioManager.OnAudioFocusChangeListener onAudioFocusChangeListener = focusChange -> {
        switch (focusChange) {
    ...
    protected void netWorkErrorLogic() {
        ...
        Log.e(TAG, "******* Net State Changed. renew player to connect *******" + currentPosition);
    ...
    protected void deleteCacheFileWhenError() {
        clearCurrentCache();
        Log.e(TAG, "Link Or mCache Error, Please Try Again " + mOriginUrl);
        if (mCache) {
            Log.e(TAG, "mCache Link " + mUrl);
        }
        mUrl = mOriginUrl;
    }
    ...
    public void onInfo(int what, int extra) {
        if (what == MediaPlayer.MEDIA_INFO_BUFFERING_START) {
            ...
            mRotate = extra;
            Log.d(TAG, "Video Rotate Info " + extra);
            ...
            mNetInfoModule = new NetInfoModule(mContext.getApplicationContext(), new NetInfoModule.NetChangeListener() {
                ...
                public void changed(String state) {
                    if (!mNetSate.equals(state)) {
                        Log.e(TAG, "******* change network state ******* + state");;
                        mNetChanged = true;
                    }
                    mNetSate = state;
                }
